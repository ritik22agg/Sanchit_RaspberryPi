...
void
serialize_string( GByteArray* b, mpz_t x)
{
	char* s;
...
GByteArray*
bswabe_prv_serialize( bswabe_prv_t* prv ,int a)
{
	GByteArray* b;
	int i;
	mpz_t userid;
	...
	serialize_string(b, userid);
	return b;
}

bswabe_prv_t*
bswabe_prv_unserialize( bswabe_pub_t* pub, GByteArray* b, int free , int* a)
{
	bswabe_prv_t* prv;
	int i; 
	int offset;
	...
	unserialize_string(b, &offset, userid);
	*a=mpz_get_ui(userid);
	if( free )
		g_byte_array_free(b, 1);
	return prv;
